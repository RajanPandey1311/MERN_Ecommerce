...
//MongoDB
mongoose.connect(
  "mongodb+srv://rajan:rajan3107@cluster0.ubuiepm.mongodb.net/e-commerce"
);
...
app.post("/upload", upload.single("product"), (req, res) => {
  res.json({
    success: 1,
    image_url: `https://mern-ecommerce-2-7wet.onrender.com/images/${req.file.filename}`,
  });
});
...
const fetchUser = async (req, res, next) => {
  ...
  if (!token) {
    res.status(401).send({ errors: "Please authentic with valid token" });
  } else {
    try {
      ...
      res
        .status(401)
        .send({ errors: "Please authentic using the valid token" });
    }
  }
};
